...
summary(fit)  
ggsurvplot(fit, pval=TRUE,conf.int=TRUE,xlab="Time in days", ggtheme=theme_classic(), censor.mark=TRUE, surv.median.line="hv",cumevents = TRUE, risk.table = TRUE, fun = "cumhaz", )
fit1 <- survfit(Surv(time,status)~prior, data=veteran)
summary(fit1)
ggsurvplot(fit1, pval=TRUE,conf.int=TRUE,xlab="Time in days", ggtheme=theme_classic(), censor.mark=TRUE, surv.median.line="hv")
